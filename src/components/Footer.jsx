        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 60, marginBottom: 70 }}>
          {[{ title:'Company', links:['About us','Our offerings','Newsroom','Investors','Blog','Careers','Uber One'] },
            ...
            <div key={col.title}>
              ...
              {col.links.map(l => <Link key={l} to={l === 'Ride' ? '/ride' : l === 'Drive' ? '/drive-page' : l === 'Eat' ? '/eats' : l === 'Uber for Business' ? '/business' : '#'} style={{ display:'block', color:'#fff', textDecoration:'none', marginBottom:16, fontSize:15 }}>{l}</Link>)}
            </div>
          ))}
        </div>
