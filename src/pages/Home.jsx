    ...
    <div>
      ...
      <section style={{ background: '#000', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '60px 120px', minHeight: 380 }}>
        <div style={{ width: '40%' }}>
          ...
          <Link to="/eats"><button style={{ background: 'white', color: 'black', border: 'none', padding: '14px 28px', borderRadius: 8, fontSize: 16, fontWeight: 600, cursor: 'pointer' }}>Try it now</button></Link>
        </div>
      ...
      <section style={{ background: '#000', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '80px 120px', gap: 80 }}>
        <div style={{ flex: 1, maxWidth: 500 }}>
          ...
          <ul style={{ marginLeft: 25, marginBottom: 40 }}>
            {['Business travel','Courtesy rides','Meal programs','Item delivery'].map(l => <li key={l} style={{ marginBottom: 25, fontSize: 22 }}><Link to={l === 'Business travel' ? '/business' : l === 'Courtesy rides' ? '/courtesy-rides' : l === 'Meal programs' ? '/meal-programs' : l === 'Item delivery' ? '/item-delivery' : '#'} style={{ color: '#fff' }}>{l}</Link></li>)}
          </ul>
      ...
      <section className="faq-section" style={{ padding: '80px 20px' }}>
        <div style={{ maxWidth: 1150, margin: '0 auto' }}>
          ...
          {[
            { q: 'Can I have a lost item delivered to me?', a: 'Yes, you can use the "Return a lost item" feature in the app to arrange delivery of a lost item. A small fee may apply based on the distance.' },
            { q: 'Can I rent a car using Uber?', a: 'Uber does not directly rent cars, but you can use Uber Rent in select cities to book a rental car through our partners.' },
            { q: 'Can I request a ride that picks up friends in different locations?', a: 'Yes, you can add multiple stops to your trip. Just tap the "+" icon next to the destination field to add pickup points for friends along the way.' },
            { q: 'Can I request a taxi on Uber?', a: 'Yes, Uber Taxi is available in select cities. You can choose the Taxi option when requesting a ride if it is available in your area.' },
            { q: 'Is there an Uber ride option for 5 people?', a: 'Yes, UberXL can accommodate up to 6 passengers and is perfect for groups or families traveling together.' },
          ].map((faq, i) => (
            <div key={i} style={{ borderBottom: '1px solid #e5e5e5' }}>
              ...
              <div style={{ maxHeight: expandedFaq === i ? 200 : 0, overflow: 'hidden', transition: 'max-height 0.4s ease', paddingLeft: 20, paddingRight: 20 }}>
                <p style={{ paddingBottom: 25, fontSize: 16, color: '#555' }}>{faq.a}</p>
              </div>
            </div>
          ))}
        </div>
